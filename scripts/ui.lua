...
local function calculateRestoration(bar, length) -- Credit to user "ownlyme" on Nexus!
	...
	if MUISmoothTransitions:get('SmoothTransitions') then
		restoration = bar.userData.lerp + incomingRestoration
		size = ((math.floor(restoration) - math.floor(bar.userData.lerp)) / bar.userData.base) * length
		position = v2((math.floor(bar.userData.lerp) / bar.userData.base) * length + 2, 0)
	else
...
local function handleBar(bar, stat, color, width)
	...
	if MUIYellowRemainder:get('YellowRemainder') then
		bar.content['remainder'].props.size = v2(math.min(calculateRemainder(bar) * length, length), width)
	else
		bar.content['remainder'].props.size = v2(0, width)
	end
	-- Set edge position to the right edge of the foreground
	bar.content['edge'].props.position = v2(bar.content['foreground'].props.size.x, 0)
